    ...
    componentDidMount = () => {
        ...
        if (this.props.players.length !== 0) {
            copyPerformances.map((el, idx) => {
                playersPerformances.push({ "goals": el.performance.goals })
            })
            
            copyMinutes.map((elm, idx) => {
                playersMinutes.push({ "minutePlays": elm.performance.minutePlays })
            })
            
            copyMinutes.map((elem, idx) => {
                playersAssists.push({ "assists": elem.performance.assists })
            })
